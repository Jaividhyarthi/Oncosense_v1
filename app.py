...
# ==================== IMAGE DIAGNOSIS PAGE ====================
    ...
    if not hybrid_available:
        st.markdown("### ⚡ Train Hybrid CNN → Quantum Pipeline")
        st.info("The image model needs to be trained first. Click the button below — no downloads needed, everything happens right here.")
        ...
        for col, (icon, label, color) in zip([col1, col2, col3, col4, col5], steps):
            with col:
                ...
                <div class="metric-card" style="border-color: {color};">
                    ...
                    <div style="color: {color}; font-weight: 600; font-size: 0.8rem; margin-top: 0.3rem; white-space: pre-line;">{label}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with st.expander("👀 Preview: What synthetic training images look like"):
            from utils.synthetic_generator import generate_preview_pair
            benign_preview, malignant_preview = generate_preview_pair()
            pc1, pc2 = st.columns(2)
            with pc1:
                st.image(benign_preview, caption="Benign — organized tubules, low cell density, small nuclei", use_container_width=True)
            with pc2:
                st.image(malignant_preview, caption="Malignant — dense cells, large nuclei, lost architecture", use_container_width=True)
            st.caption("These synthetic H&E-stained images mimic real histopathology slides. For production, train on real BreakHis dataset images.")

        # Training config
        st.markdown("**Training Configuration:**")
        tc1, tc2, tc3 = st.columns(3)
        with tc1:
            n_images = st.selectbox("Images per class", [100, 150, 200, 300], index=1, help="More images = better accuracy but slower training")
        with tc2:
            q_train = st.selectbox("Quantum train samples", [60, 80, 100, 120], index=2, help="Samples for quantum kernel computation")
        with tc3:
            cnn_model = st.selectbox("CNN Backbone", ["resnet18"], index=0, help="ResNet18 — best speed/accuracy balance")

        # TRAIN BUTTON
        if st.button("🚀 Train Image Model (One-Click)", type="primary", use_container_width=True):
            progress = st.progress(0)
            status = st.empty()
            ...
            try:
                ...
                status.markdown('<div class="processing-step">🎨 Generating synthetic histopathology images...</div>', unsafe_allow_html=True)
                progress.progress(5)

                from utils.synthetic_generator import generate_synthetic_dataset
                images, labels = generate_synthetic_dataset(n_benign=n_images, n_malignant=n_images)

                status.markdown(f'<div class="processing-step">✅ Generated {len(images)} images ({n_images} benign + {n_images} malignant)</div>', unsafe_allow_html=True)
                progress.progress(15)

                # Step 2: CNN Feature Extraction
                status.markdown('<div class="processing-step">🧠 Loading ResNet18 and extracting features...</div>', unsafe_allow_html=True)
                progress.progress(20)
                ...
                for i, img in enumerate(images):
                    feat = extractor.extract_single(img)
                    features_list.append(feat)
                    if (i + 1) % 50 == 0:
                        pct = 20 + int(50 * (i + 1) / total)
                        progress.progress(min(pct, 70))
                        status.markdown(f'<div class="processing-step">🧠 Extracting features: {i+1}/{total} images...</div>', unsafe_allow_html=True)

                features = np.array(features_list)
                status.markdown(f'<div class="processing-step">✅ Feature extraction complete — {features.shape[1]} features per image</div>', unsafe_allow_html=True)
                progress.progress(70)

                # Step 3: Train Hybrid Quantum Pipeline
                status.markdown('<div class="processing-step">⚛️ Training Quantum Kernel SVM pipeline...</div>', unsafe_allow_html=True)
                progress.progress(75)

                from utils.hybrid_quantum_pipeline import HybridQuantumPipeline
                pipeline = HybridQuantumPipeline(
                    n_components=4, n_qubits=4, reps=2,
                    quantum_train_size=q_train
                )

                # Redirect prints to capture metrics
                ...
                progress.progress(100)
                status.markdown('<div class="processing-step">💾 Model saved! Ready for diagnosis.</div>', unsafe_allow_html=True)

                # Show metrics
                st.success("Training complete! The Image Diagnosis tab is now ready.")
                ...
                with mc1:
                    ...
                    <div class="metric-card">
                        <div style="color: #9B59B6; font-weight: 700;">⚛️ Quantum SVM</div>
                        <div class="metric-value">{metrics['quantum']['accuracy']:.2%}</div>
                        <div class="metric-label">Accuracy | AUC: {metrics['quantum']['auc_roc']:.4f}</div>
                    </div>
                    """, unsafe_allow_html=True)
                with mc2:
                    st.markdown(f"""
                    <div class="metric-card">
                        <div style="color: #667eea; font-weight: 700;">📐 Linear SVM</div>
                        <div class="metric-value">{metrics['linear']['accuracy']:.2%}</div>
                        <div class="metric-label">Accuracy | AUC: {metrics['linear']['auc_roc']:.4f}</div>
                    </div>
                    """, unsafe_allow_html=True)
                with mc3:
                    st.markdown(f"""
                    <div class="metric-card">
                        <div style="color: #38ef7d; font-weight: 700;">🌀 RBF SVM</div>
                        <div class="metric-value">{metrics['rbf']['accuracy']:.2%}</div>
                        <div class="metric-label">Accuracy | AUC: {metrics['rbf']['auc_roc']:.4f}</div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    # ===== DIAGNOSIS SECTION (model trained) =====
    if hybrid_available:
        st.success("✅ Hybrid CNN → Quantum model is trained and ready!")

        # Option to retrain
        with st.expander("🔄 Retrain model with different settings"):
            if st.button("🗑️ Delete trained model and retrain", type="secondary"):
                import glob
        ...
        if uploaded_file is not None:
            ...
            with col2:
                ...
                <div class="metric-card">
                    ...
                    <div style="color: #ccc; font-size: 0.9rem;">
                        ...
                        Image → ResNet18 → 512 features<br>
                        → PCA → 4 features → Quantum Kernel → SVM
                    </div>
                </div>
                """, unsafe_allow_html=True)
            ...
            if st.button("⚛️ Run Hybrid Quantum Analysis", type="primary", use_container_width=True):
                with st.spinner(""):
                    ...
                    status.markdown('<div class="processing-step">🧠 Extracting ResNet18 features...</div>', unsafe_allow_html=True)
                    progress.progress(20)
                    ...
                    status.markdown(f'<div class="processing-step">✅ Extracted {len(cnn_features)} deep features</div>', unsafe_allow_html=True)
                    progress.progress(40)

                    status.markdown('<div class="processing-step">⚛️ Loading quantum pipeline & classifying...</div>', unsafe_allow_html=True)
            ...
            # No image uploaded — show architecture
            st.markdown("### 🏗️ Hybrid CNN → Quantum Architecture")

            st.markdown("""
            **Stage 1 — CNN Feature Extraction (Classical)**
            - Pretrained **ResNet18** processes the histopathology image
            - Final classification layer removed — outputs **512-dim feature vector**
            - Captures cell morphology, tissue structure, staining patterns

            **Stage 2 — Quantum Classification**
            - Features normalized (MinMaxScaler) and reduced (PCA → 4 components)
            - **ZZFeatureMap** encodes into quantum states |psi(x)>
            - **Fidelity Quantum Kernel** computes K(x,x') = |<psi(x)|psi(x')>|^2
            - **SVC** classifies using precomputed quantum kernel

            **Why this is novel:** Most quantum ML papers use tabular data. This pipeline
            demonstrates quantum classification on **real medical images** through a CNN
            feature bridge — the CNN handles pixels, the quantum kernel handles classification.
            """)
